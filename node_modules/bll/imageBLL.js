...
var checkIfEnd = function(images) {
    images.isEnd = false;
    if(images) {
        if(images.length <= numberPerPage) {
            images.isEnd = true;
        } else {
            images.pop();
        }
...
exports.getImages = function(req, res, callback) {
    var preId = req.query.end_id;
    var nextId = req.query.start_id;
    var pageDis = req.query.pdis;
    if(!posint.test(pageDis)) {
        pageDis = 1;
    }
    if(preId) {
        if(preId.length != 24) {
        ...
        imageDAL.findNextPageImagesSortByCreateTime(preId, pageDis, numberPerPage, function(err, images) {
            if(err) {
        ...
        imageDAL.findPrePageImagesSortByCreateTime(nextId, pageDis, numberPerPage, function(err, images) {
            if(err) {
                SLOG.error("Get images error: ", err);
                res.send(errorDef[400002], 500);
            } else {
...
exports.removeImage = function(req, res, callback) {
    var id = req.query.id;
    if(id.length != 24) {
        res.send(errorDef[400008], 500);
        return;
    ...
    imageDAL.findById(id, function(err, image) {
        if(err || !image) {
            ...
            if(req.session.user && req.session.user._id == image.uid) {
                upyunws.delete(image.url, function(err, body) {
                    if(err) {
                        SLOG.error("Delete upyun image error: ", err);
                    } else {
                        SLOG.trace("Delete upyun image successfully: ", body);
                    }
                });
                imageDAL.removeById(id, callback);
            } else {
